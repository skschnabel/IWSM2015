...
of genetic values \ref{Heffner2009,Jannink2010} and the detection of quantitative trait loci (QTL) 
\ref{Bernardo2008}. Genome-wide association studies (GWAS) have emerged as a great tool for the 
localization of QTL in breeding programs with material of varying degrees of population structure
\ref{Bush2012}. In order to reduce the number of false-positive signals for marker-trait associations,
such structure needs to be accounted for \ref{Astle2009}. This problem is typically addressed through 
the use of kinship matrices \textbf{K}, which are estimated from molecular marker data \ref{VanRaden2008}.
...
markers \ref{Hill1968}. However, the power at a SNP decreases when it, or a marker in LD therewith,
is simultaneously used for the computation of \textbf{K} as well as for testing marker-trait 
associations \ref{Listgarten2012}. Hence, we are addressing this problem by visualizing the distance
between pairs of markers at which they are not considered to be in LD anymore. Such markers can be 
selected by researchers for the computation of \textbf{K}. Moreover, this method can aid the elucidation
...
\ref{Fischer2008}. Figure~\ref{SchnabelTorrettaWesthues:fig1}~(A) shows the typical LD decay plot 
...
As a recurring example in this manuscript we are using Chromosome 1 of the above described 
	...
	$\mu_{\tau}=s_{\tau}(dist)$, where $\mu_\tau$ is the quantile function at percentile $\tau$, $dist$ is the SNP distance between couples of markers and $s_{\tau}(\cdot)$ is a smooth and unknown function.
	...
	of \textbf{K}.
	On Chromosome 1 of this population we have 1085 sliding windows \textcolor{blue}
	{we have 1119 sliding windows in the main dataset but we actually used 1085 sliding windows after the corrections
}
	 of 2.5Mbp with an 
